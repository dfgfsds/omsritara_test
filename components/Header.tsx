  ...
  return (
    <>
      <header className="bg-white">
        ...
        <div className="flex items-center justify-between py-3 mx-6 lg:mx-10">
          ...
          <div className="hidden lg:flex items-center gap-6 text-sm">
            ...
            <Link href="/cart" className="relative flex items-center gap-1">
              ...
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-3 h-4 w-4 text-[10px] bg-red-600 text-white rounded-full flex items-center justify-center">
                  {cartCount}
                </span>
              )}
            </Link>
            ...
            <div ref={currencyRef} className="relative">
              ...
              {currencyOpen && (
                <div className="absolute mt-2 bg-white border rounded shadow w-24">
                  {["INR", "USD"].map((c) => (
                    <button
                      key={c}
                      onClick={() => {
            ...
            <div ref={userMenuRef} className="relative">
              {userName ? (
                ...
                <button
                  onClick={() => setSignInModal(true)}
                  className="bg-[#a5291b] text-white px-4 py-2 rounded flex items-center gap-1"
                >
                  <User size={18} /> Login
                </button>
              )}
        ...
        {/* DESKTOP NAV */}
        <nav className="hidden lg:flex bg-[#a5291b] mx-6 lg:mx-10 rounded-md text-white justify-evenly py-2">
          {[
            ["/", "Home"],
            ["/shop", "Shop"],
            ["/categories", "Categories"],
            ["/shopByIntention", "Shop by Intention"],
            ["/combo-offers", "Offers"],
            ["/blog", "Blog"],
            ...
            <Link
              key={url}
              href={url}
              className={isActive(url) ? "underline" : ""}
            >
              {label}
            </Link>
          ))}
